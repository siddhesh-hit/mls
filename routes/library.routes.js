...
router.route('/')
    ...
    .post(authMiddleware, checkRoleMiddleware(['Admin']), upload.single('banner'), createLibrary);

router.route('/:id')
    .get(authMiddleware, checkRoleMiddleware(['Admin']), getLibrary)
    .put(authMiddleware, checkRoleMiddleware(['Admin']), upload.single('banner'), updateLibrary)
